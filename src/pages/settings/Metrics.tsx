...
export default function Metrics() {
  ...
  const { inicioStr, fimStr } = getInicioFimMes(year, Number(currentMonthNumber));
  const [inicio, setInicio] = useState<string | Date>(inicioStr);
  const [fim, setFim] = useState<string | Date>(fimStr);
  const { data: metricsData, isLoading, error } = useMetricsList({
  ...
  const consult = ({type,value,inicio,fim}) => {
    ...
    // setMetricsData(metricsData?.registros ?? []);
  ...
  const metrics = resAny?.registros ?? resAny?.data ?? [];
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              {["investment","visitors","bot_conversations","human_conversations","proposals","closed_deals"].map((name) => (
